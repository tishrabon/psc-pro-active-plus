...
const Ambience = () => {
  ...
  const fetchElement = async () => {
    try {
      if (se.selected && clockData.occupied) {
        ...
        if (!clockData.paused && !clockData.completed && (clockData.running === "work" || clockData.running === "break")) {
          const response = await fetch(`https://freesound.org/apiv2/sounds/${se.elementId}/?token=${soundKey}`);
          const data = await response.json();
    
          if (data) {
            setSoundUrl(data.previews["preview-hq-mp3"]);
            console.log("sound fetched from api");
            localStorage.setItem(`element-${se.elementId}`, data.previews["preview-hq-mp3"]);
          }
        }
      }
    }
  ...
  useEffect(() => {
    if (se.selected && !clockData.paused && !clockData.completed && soundUrl && (clockData.running === "work" || clockData.running === "break")) {      
  ...
  }, [se.selected, se.volume, clockData.running, clockData.paused, clockData.completed, soundUrl]);
  ...
  useEffect(() => {
    ...
    if (mn.selected && !clockData.paused && !clockData.completed && (clockData.running === "work" || clockData.running === "break")) {
