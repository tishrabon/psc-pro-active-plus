    ...
    <div className="otherpage-container verticalS mt-[32px] gap-5">
      ...
      <div className="verticalS authbox">
        <form onSubmit={exeSignup} action="" className="verticalC gap-5 py-5">
          <input 
            type="text" placeholder="What should we call you?" value={name} required            
          ...
          />
          <input 
            type="password" placeholder="New Password" value={psw} required            
            onChange={(e) => { setPsw(e.target.value) }}
            className="border border-gray-300 rounded-sm p-2 text-sm"
          />  
          <input 
            type="password" placeholder="Confirm Password" value={confirmPsw} required            
            onChange={(e) => { setConfirmPsw(e.target.value) }}
            className="border border-gray-300 rounded-sm p-2 text-sm"
          />              

          <button type="submit" className="py-2 px-5 rounded-sm bg-material text-white">Sign Up</button>   
        </form>    
      </div> 

      <div className="p-5 text-center">          
        <div className="border-b border-gray-300 mb-3 pb-3">Go to <Link to="/Login" className="text-green-600 hover:underline">Login</Link> Page</div>          
      </div>               
    </div>
  )
};
