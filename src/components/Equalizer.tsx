...
const Equalizer = () => {
  ...
  return (
    <div className="verticalS">
          ...
          <div className="flex flex-col justify-start items-center flex-1">
            ...
            <div className="verticalC gap-1">
              <div className="text-[12px]">
                Cycle: {appData.pomodoro.formatt}
              </div>
              <div className="text-[12px] horizontalC gap-1">
                <ImLoop2 size={10} /> Loop: {appData.pomodoro.sessions}
              </div>
              <div className="text-[12px] horizontalC gap-1">
                <MdOutlineTimer size={14} /> {timeHrMin(duration)}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col justify-between items-center flex-1">
            ...
            <div className="w-full grid grid-cols-[20px_1fr] items-center gap-1 border-b border-white pb-2 mb-2">
              <button
                title="Check to play"
                onClick={() => {
                  dispatch(selectAMBIENCE("metronome"));
                }}
              >
                {appData.ambience.metronome.selected ? (
                  <FaCircleCheck />
                ) : (
                  <ImRadioUnchecked />
                )}
              ...
              <div>
                <div>
                  ...
                  <div className="text-[12px]">
                    Bpm: {appData.ambience.metronome.bpm}
                  </div>
                </div>
            ...
            <div className="w-full grid grid-cols-[20px_1fr] items-center gap-1">
              <button
                title="Check to play"
                onClick={() => {
                  dispatch(selectAMBIENCE("serenity"));
                }}
              >
                {appData.ambience.serenityElements.selected ? (
                  <FaCircleCheck />
                ) : (
                  <ImRadioUnchecked />
                )}
              ...
              <div>
                <div>
                  ...
                  <div className="text-[12px]">
                    {appData.ambience.serenityElements.element
                      ? appData.ambience.serenityElements.element
                      : "Pls Select Element"}
                  </div>
                </div>
      ...
      {/* pomodoro modal  */}
      <Modal open={pomodoroModal} exeClose={() => setPomodoroModal(false)}>
        <div className="verticalS gap-5">
          <h1 className={modalHeadStyles}>Customize Pomodoro</h1>
          {/* formatt */}
          <p className="text-[12px]">
            Formatt: {appData.pomodoro.formatt}, Session:{" "}
            {appData.pomodoro.sessions}, Duration:{" "}
            {duration ? timeHrMin(duration) : "Not Set"}
          </p>
          <p className="text-[12px] mt-[-10px] border-b border-gray-300 pb-2 mb-1">
            {appData.pomodoro.formatt
              ? foucsVsBreak(
                  appData.pomodoro.formatt,
                  appData.pomodoro.focusName,
                  appData.pomodoro.breakName,
                )
              : "Check The Settings Please"}
          </p>

          <div className="verticalC">
            <p className="w-[200px] border border-gray-400 p-1 my-1 rounded-[5px] text-[14px]">
              Cycle:{" "}
              {appData?.pomodoro?.formatt
                ? appData.pomodoro.formatt
                : "Select Formatt"}
            ...
            <select
              ...
              onChange={(e) => {
                dispatch(setFORMATT(e.target.value));
              }}
          ...
          <div className="verticalC">
            <p className="w-[200px] border border-gray-400 p-1 text-[14px] my-1 rounded-[5px]">
              Session Count: {appData?.pomodoro?.sessions ?? "Set"}
            </p>

            <div className="flex flex-wrap gap-5 mt-2">
              <button
                className="w-[35px] h-[35px] text-[12px] p-1 rounded-[5px] bg-second active:bg-gray-300"
          ...
          <div className="verticalC gap-1">
            ...
            >
              <input
                ...
                onChange={(e) => {
                  setFocusTimeInput(Number(e.target.value));
                }}
              />
              <p>/</p>
              <input
                ...
                className="p-1 border border-gray-300 h-[30px] w-[60px] rounded-[5px] text-center text-[14px]"
                onChange={(e) => {
                  setBreakTimeInput(Number(e.target.value));
                }}
              />
              <button
                type="submit"
            ...
            <div className="verticalC gap-1">
              ...
              <div className="horizontalC gap-2">
                <input
                  ...
                  onChange={(e) => {
                    setFocusNameInput(e.target.value);
                  }}
                />
                <button
                  className="h-[30px] w-[60px] rounded-[5px] bg-second text-[14px]"
                  onClick={() => {
                    if (focusNameInput !== "") {
                      if (focusNameInput.length >= 3) {
                        dispatch(
                          updateLABEL({ cmd: "focus", label: focusNameInput }),
                        );
                        setFocusNameInput("");
                      } else {
            ...
            <div className="verticalC gap-1">
              ...
              <div className="horizontalC gap-2">
                <input
                  ...
                  className="block p-1 border border-gray-300 h-[30px] w-[130px] rounded-[5px] text-center text-[14px]"
                  onChange={(e) => {
                    setBreakNameInput(e.target.value);
                  }}
                />
                <button
                  className="h-[30px] w-[60px] rounded-[5px] bg-second text-[14px]"
                  onClick={() => {
                    if (breakNameInput !== "") {
                      if (breakNameInput.length >= 3) {
                        dispatch(
                          updateLABEL({ cmd: "break", label: breakNameInput }),
                        );
                        setBreakNameInput("");
                      } else {
                        alert("Label must be at least 3 characters long.");
      ...
      <Modal open={ambienceModal} exeClose={() => setAmbienceModal(false)}>
        ...
        <div className="border-b border-gray-300 pb-5 mb-5 verticalS gap-3">
          ...
          <div>
            <p className="w-[200px] border border-gray-400 p-1 my-1 rounded-[5px] text-[14px]">
              Bpm:{" "}
              {appData.ambience.metronome.bpm
                ? appData.ambience.metronome.bpm
                : appData.ambience.metronome.bpm}
            </p>
            <select
          ...
          <div>
            <p className="w-[200px] border border-gray-400 p-1 my-1 rounded-[5px] text-[14px]">
              ...
              {appData.ambience.metronome.sound === "tick-classic.wav" &&
                "Classic"}
              {appData.ambience.metronome.sound === "tick-drum.wav" &&
                "Smooth Drum"}
            </p>
            <select
              className="bg-second p-1 rounded-[5px] mt-1 text-[14px]"
              onChange={(e) => {
                dispatch(setTICK(e.target.value));
              }}
        ...
        <div className="pb-5 mb-5 verticalS gap-3">
          ...
          <div>
            ...
            <select
              className="bg-second p-1 rounded-[5px] mt-1 text-[14px]"
              onChange={(e) => {
                dispatch(setELEMENT(e.target.value));
              }}
